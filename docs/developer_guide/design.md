...
### 参加者モード（URLベース）
- 参加者向けには `/?eventId=<イベントID>` の形式でリンクを配布する。
- フロントはクエリの `eventId` を読み取り、参加者モードでは「採点」「結果」以外のUI（イベント作成、イベント参加の独立セクション）を非表示にする。

#### 参加者向けリンク（次回は即採点）
- 参加完了後に、参加者自身が再訪するためのリンクを表示する。
- `eventId` はルーティングのためクエリに含める。
- 参加者名はログ等へ残りにくいよう **URLのhash（フラグメント）** で受け渡しし、初回ロード時に自動joinへ利用する。
